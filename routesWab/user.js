...
router.post('/login', function(req, res, next) {
  ...
  result.then(data => {
    if (data) {
      res.json(new this.SuccessModel(data))
    } else {
      res.json(new this.ErrorModel(data, '操作失败'))
    }
  })
})
...
router.get('/userinfo', function(req, res, next) {
  ...
  result.then(data => {
    if (data) {
      ...
      res.json(new this.SuccessModel(data[0]))
    } else {
      res.json(new this.ErrorModel(data, '操作失败'))
    }
  })
...
router.post('/buy', function(req, res, next) {
  ...
  result
    .then(data => {
      res.json(new this.SuccessModel(data, data.message))
    })
    .catch(err => {
      res.json(new this.ErrorModel(err, err.message))
    })
})
